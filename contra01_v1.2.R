...
for (i in 1:nrow(niftysymbols)) {
        ...
        if (file.exists(paste(datafolder, symbol, ".Rdata", sep = ""))) {
                ...
                if (nrow(md) > MALongPeriod) {
                        ...
                        md <-
                                md[md$date >= BackTestStartDate & md$date <= BackTestEndDate, ]
                        allmd[[i]] <- md
                        signals <-
                                rbind(md[, c(
                                        "date",
...
if (writeToRedis) {
        ...
        #vsell = processedsignals[(nrow(processedsignals) - nrow(niftysymbols)):nrow(processedsignals), c("symbol", "sell")]
        vsell = processedsignals[processedsignals$date==today, c("symbol", "sell")]
        ...
        #vbuy = processedsignals[(nrow(processedsignals) - nrow(niftysymbols)):nrow(processedsignals), c("symbol", "buy")]
        vbuy = processedsignals[processedsignals$date==today, c("symbol", "buy")]
...
gettradecount <- function(trades) {
        ...
        for (i in 1:length(psymbol)) {
                tradessubset <-
                        trades[trades$exitprice == 0 & trades$symbol == psymbol[i], ]
                out = c(out, nrow(tradessubset))
        }
        out
}

print(trades[trades$exitprice == 0 |
                     as.Date(trades$exittime, tz = "Asia/Kolkata") == as.Date(today, tz = "Asia/Kolkata"), ])
psymbol = unique(trades[trades$exitprice == 0, c("symbol")])
...
if(length(pindex)>0){
        summary <-
                data.frame(
                        symbol = psymbol,
                        position = getposition(allequity, pindex),
                        tradecount = gettradecount(trades)
                )
        print(summary)
